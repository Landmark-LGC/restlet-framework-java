...
/**
 * The Class AtomFeedHandler which parses the AtomFeed using STAX Event Iterator
 * model.
 * 
 * @param <T>
 *            the generic type
...
public class AtomFeedHandler<T> extends XmlFormatParser implements
	 ...
	 *            the entity class
	 */
	public AtomFeedHandler(String entitySetName, EntityType entityType,
			Class<?> entityClass) {
		...
		try {
			...
			while (eventReader.hasNext()) {
				...
				if (isStartElement(event, ATOM_ENTRY)) {
					...
					this.parseEntry(eventReader, event.asStartElement(),
							entitySet, entity);
	...
	private void parseAtomFeedLinks(XMLEvent event) {
		...
		if (null != event.asStartElement()
			...
			String type = event.asStartElement()
					.getAttributeByName(new QName("type")).getValue();
			if (type != null && type.length() > 0) {
				link.setType(new MediaType(type));
			}
		}

		if (null != event.asStartElement().getAttributeByName(
				new QName("hreflang"))) {
			link.setHrefLang(new Language(event.asStartElement()
					.getAttributeByName(new QName("hreflang")).getValue()));
		}
		...
		if (null != event.asStartElement().getAttributeByName(
			...
			final String attr = event.asStartElement()
					.getAttributeByName(new QName("length")).getValue();
	 ...
	 * @return the t
	 */
	public static <T> T parseProperties(XMLEventReader reader,
			StartElement propertiesElement, T entity) {
		...
		try {
			...
			while (reader.hasNext()) {
				...
				if (event.isStartElement()
					...
					// Check if that property is java reserved key word. If so
					// then prefix it with '_'
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static <T> void parsePropertiesByType(XMLEventReader reader,
			T entity, String propertyName, XMLEvent event)
			throws XMLStreamException, Exception {
		Object value = null;
		Attribute typeAttribute = event.asStartElement().getAttributeByName(
				M_TYPE);
		Attribute nullAttribute = event.asStartElement().getAttributeByName(
				M_NULL);
		...
		if (typeAttribute == null) { // Simple String
			...
			CollectionPropertyHandler.parse(reader, o, event.asStartElement(),
					entity);
	 ...
	 *            the entity
	 */
	private void parseDSAtomEntry(EntityType entityType, XMLEventReader reader,
			XMLEvent event, T entity) {
		// as end element is not included in parseProperties, we need a wrapper
		// method around it to handle it.
	...
	private T parseEntry(XMLEventReader reader, StartElement entryElement,
		...
		if (null == baseURL) {
			Attribute attributeByName = entryElement
					.getAttributeByName(XML_BASE);
			if (attributeByName != null) {
				baseURL = attributeByName.getValue();
			}
			...
			try {
				...
				if (event.isEndElement()
					...
					if (p != null) {
						...
						setDetailsOfPerson(entity, p, null); // third parameter
																// would be null
																// if we don't
																// have
																// contributor
				...
				if (isStartElement(event, ATOM_ID)) {
					 ...
					 * .findFunctionImport(function.title, entitySet.getType(),
					 * FunctionKind.Action)); } else if (isStartElement(event,
					 * M_FUNCTION)) { AtomFunction function =
					 * parseAtomFunction(reader, event.asStartElement());
					...
					contentType = getAttributeValueIfExists(
							event.asStartElement(), "type");
					relativeURL = getAttributeValueIfExists(
							event.asStartElement(), "src");
					if (MediaType.APPLICATION_XML.getName().equals(contentType)) {
							...
							try {
								...
								if (valueElement == null
									...
									if (isStartElement(event2, M_PROPERTIES)) {
										this.parseDSAtomEntry(
												entitySet.getType(), reader,
												event2, entity);
									} else {
										// TODO: Onkar : Set Basic content by
										// implementing innerText method later
										// BasicAtomEntry bae = new
										// BasicAtomEntry();
										Entry bae = new Entry();
										// bae.content = innerText(reader,
										// event2.asStartElement());
										rt = bae;
									}
								}
						...
						if (entityType.isBlob()) {
							for (Field field : entity.getClass()
								...
								if (type.getName().contains("StreamReference")) {
									Reference baseReference = new Reference(
											baseURL);
									StreamReference streamReference = new StreamReference(
											baseReference, relativeURL);
						...
						Entry e = new Entry();
						// TODO: Onkar : Set Basic content by implementing
						// innerText method later
						// e.setContent(innerText(reader,
						// event.asStartElement()));
						rt = e;
					}
				} else if (event.toString() != null
						&& event.toString().isEmpty()
						&& event.toString().trim().isEmpty()) {
					continue;
				} else { // Handle Custom feeds where some properties are
							// outside content tag. ie. not in m:properties
					if (event.isStartElement()) {
						...
						while (reader.hasNext()) {
							...
							try {
								// iterate all the sub-elements under parent tag
								// with
								event2 = reader.nextEvent();
								if (event2.isEndElement()
								...
								if (valueElement == null
										&& event2.isStartElement()) {
									valueElement = event2.asStartElement();
									// Iterate through sub-paths and set
									// respective property for the entity.
									// Eg:<cafe:Company><cafe:Name>Cafe
									// corp.</cafe:Name></cafe:Company>
									String innerTag = parentTag
											+ "/"
											+ valueElement.getName()
													.getLocalPart();
									for (Iterator iterator = mappings
											.iterator(); iterator.hasNext();) {
										Mapping mapping = (Mapping) iterator
												.next();
										if (mapping.getValuePath()
												.equalsIgnoreCase(innerTag)) {

											// TODO: Abhijeet : Extract this
											// into method and refer it in
											// parseProperties
											propertyName = ReflectUtils
													.normalize(mapping
															.getPropertyPath());
											parsePropertiesByType(reader,
													entity, propertyName,
													event2);
											break; // exit for loop if mapping
													// is present and addressed
	...
	private void setDetailsOfPerson(T entity, Person author, Person contributor) {
		...
		for (Mapping m : metadata.getMappings()) {
			if (entityType != null && entityType.equals(m.getType())
				 ...
				 * value = entry.getPublished(); } else if
				 * ("SyndicationRights".equals(m.getValuePath())) { value =
				 * (entry.getRights() != null) ? entry.getRights() .getContent()
				 * : null; } else if
				 * ("SyndicationSummary".equals(m.getValuePath())) { value =
				 * entry.getSummary(); } else if
				 * ("SyndicationTitle".equals(m.getValuePath())) { value =
				 * (entry.getTitle() != null) ? entry.getTitle() .getContent() :
				 * null; } else if
				 * ("SyndicationUpdated".equals(m.getValuePath())) { value =
				 * entry.getUpdated(); }
				 */
	...
	private Link parseAtomLink(XMLEventReader reader, StartElement linkElement,
		...
		try {
			...
			// 2. <m:inline/></link> - inlined content but null entity or empty
			// feed
			// 3. <m:inline><feed>...</m:inline></link> - inlined content with 1
			// or more items
			// 4. <m:inline><entry>..</m:inline></link> - inlined content 1 an
			// item
			...
			while (reader.hasNext()) {
				...
				if (event.isEndElement()
					...
					// skip all tags until we encounter first entry element to
					// exclude link and other tags.
					// Fix for edit link tag to prevent pre-matured exit of
					// parent link iteration.
					while (reader.hasNext()) {
						event = reader.nextEvent();
						if (isStartElement(event, ATOM_ENTRY)) {
							...
							Object o = ReflectUtils.getPropertyObject(entity,
									propertyName);
							parseInlineEntities(reader, entitySet, entity,
									event, propertyName, o);
							// This break is added to not handle additional
							// inline entities here. Those entries shall be
							// handled in next else-if block.
							break;
						} else if (event.isEndElement()
								&& event.asEndElement().getName()
										.equals(ATOM_FEED.getLocalPart())) {
							break;
						}
					}
				} else if (isStartElement(event, ATOM_ENTRY)) { // handle the
																// inline entity
					String propertyName = rt.getHref().getLastSegment();
					// create a property object
					Object o = ReflectUtils.getPropertyObject(entity,
							propertyName);
					parseInlineEntities(reader, entitySet, entity, event,
							propertyName, o);
				}
			}
			return rt;
	...
	private void parseInlineEntities(XMLEventReader reader,
			EntitySet entitySet, T entity, XMLEvent event, String propertyName,
			Object o) {
		try {
			if (o instanceof List) { // Collection of complex i.e. one to many
										// association
				Field field = entity.getClass().getDeclaredField(
						ReflectUtils.normalize(propertyName));
				// String currentMType =
				// startElement.getAttributeByName(M_TYPE).getValue();
				...
				if (field.getGenericType() instanceof ParameterizedType) {
					...
					this.parseEntry(reader, event.asStartElement(), entitySet,
							(T) obj);
					((List) o).add(obj);
				}
